                        ...
                        if (state is ChatLoadedState) {
                          ...
                          return ListView.builder(
                              ...
                              itemBuilder: (context, index) {
                                if (index == 0) {
                                  // Nút "Load Old Messages" ở đầu danh sách
                                  return TextButton(
                                    onPressed: () {
                                      // Xử lý sự kiện tải tin nhắn cũ
                                      loadOldMessage(
                                          state.messages[index].idMessage,
                                ...
                                if (index == state.messages.length - 1) {
                                  // Widget cuối cùng, đóng vai trò là footer
                                  return ElevatedButton(
                                    onPressed: () {
                                      // Xử lý sự kiện tải tin nhắn mới
                                      sl<RoomChatBloc>().add(
                                          LoadMessagesNewEvent(
                                              idRoom: widget.rc.idRoom));
                                    },
                                    child: Text("Load New Messages"),
                                  );
                                }
                                bool check =
                                    state.messages[index - 1].userId == _userId;
                                return ChatContent(
                                    state.messages[index - 1].userId,
                                    state.messages[index - 1].message,
                                    check);
                              });
                        }
                        return SizedBox();
