...
class _ProfilePageState extends State<ProfilePage>
  ...
  Future<void> _loadDefault() async {
    ...
    setState(() {
      int roleDefault = widget.role ?? prefs.getInt('role') ?? 0;
      int userIDDefault =
          widget.userIDToLoadProfile ?? prefs.getInt('userID') ?? 0;
      sl<ProfileBloc>()
          .add(LoadProfile(role: roleDefault, userID: userIDDefault));
      logger.d("At Profile Page: ${roleDefault} ${userIDDefault}");
    });
  }
                ...
                child: Column(
                  children: [
                    ...
                    BlocBuilder<ProfileBloc, ProfileState>(
                      builder: (context, state) {
                        if (profile.user is DepartmentModel) {
                          final department = profile.user as DepartmentModel;
                          return BioUser(
